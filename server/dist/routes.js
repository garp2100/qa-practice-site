...
router.post('/items', authMiddleware, async (req, res) => {
    const userId = req.userId;
    const { name } = req.body;
    const { rows } = await query('insert into items (owner_id, name) values ($1, $2) returning *', [userId, name]);
    res.json(rows[0]);
});
router.patch('/items/:id', authMiddleware, async (req, res) => {
    ...
    const { id } = req.params;
    const { done } = req.body;
    const { rows } = await query('update items set done=$1 where id=$2 and owner_id=$3 returning *', [done, id, userId]);
    res.json(rows[0]);
});
router.delete('/items/:id', authMiddleware, async (req, res) => {
    const userId = req.userId;
    const { id } = req.params;
    await query('delete from items where id=$1 and owner_id=$2', [id, userId]);
    res.json({ ok: true });
});
